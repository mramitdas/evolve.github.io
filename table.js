...
async function fetchTableData() {
  ...
  try {
    ...
    data.forEach((item, index) => {
      ...
      if (mobileList) {
        const card = `
          <div class="bg-gray-800 rounded-xl p-4 shadow flex items-center gap-4">
              <div class="text-gray-400 font-bold text-lg w-6">${index + 1}</div>

              <img src="${imgUrl}" class="w-12 h-12 rounded-full object-cover"/>

              <div class="text-left">
                  <p class="text-white font-semibold text-lg">${item.name}</p>
                  <p class="text-gray-400 text-sm">${item.phone_number}</p>
                  <p class="${
                    isActive ? "text-green-400" : "text-red-400"
...
window.filterTable = function () {
  ...
  rows.forEach((row) => {
    ...
    row.style.display =
      name.includes(query) || phone.includes(query) ? "" : "none";
  });

  // ✅ Mobile cards
  const cards = document.querySelectorAll("#mobileList > div");
  cards.forEach((card) => {
    const name = card.querySelector(".text-white")?.textContent.toLowerCase() || "";
    const phone = card.querySelector(".text-gray-400")?.textContent.toLowerCase() || "";

    card.style.display =
      name.includes(query) || phone.includes(query) ? "flex" : "none";
  });
};
